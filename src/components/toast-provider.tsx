...
interface ToastEntry { message: string; type: ToastType }
...
const TOAST_THEME: Record<ToastType, { bg: string; border: string; icon: string; text: string }> = {
  ...
  info:    { bg: '#fff8e6', border: '#ffd966', icon: '🦊', text: '#7a4f00' },
};
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const showConfirm = useCallback((options: ConfirmOptions) => setConfirm(options), []);
  ...
  return (
    <ToastContext.Provider value={{ showToast, showConfirm }}>
      ...
      <Modal visible={!!confirm} transparent animationType="slide" statusBarTranslucent onRequestClose={() => setConfirm(null)}>
        <View style={s.container}>
          ...
          <View style={s.sheet}>
            ...
            <View style={s.sheetBtns}>
              ...
              <Pressable style={[s.confirmBtn, confirm?.destructive && s.destructiveBtn]} onPress={handleConfirm}>
...
function makeStyles(c: AppColors) {
  return StyleSheet.create({
    ...
    sheetBar: {
      ...
      backgroundColor: c.borderMid, marginBottom: 20,
    },
    ...
    confirmBtn: { flex: 1, paddingVertical: 14, borderRadius: 12, backgroundColor: '#2E7D32', alignItems: 'center' },
